 ...
 *
 * 概述：
 *   虚拟键码（Virtual-Key Codes, VK_*）定义
 *
 * 说明：
 *   这些常量用于标识键盘输入中的逻辑按键，由 Windows 输入系统定义。
 *   与 macOS 的虚拟键码（基于物理按键位置）不同，Windows 的虚拟键码主要表示“键的语义或功能”，
 *   而不是具体的物理键位。
 *
 *   例如，VK_A 表示字母 'A' 键的输入语义，而不是某一个固定的物理按键位置；
 *   在不同键盘布局（如 QWERTY、AZERTY）下，产生 'A' 的物理按键可能不同，
 *   但都会映射为相同的 VK_A。
 *
 *   某些按键（如 Shift、Ctrl、Alt）还区分左右键（如 VK_LSHIFT / VK_RSHIFT），
 *   但这种区分依赖于具体的输入事件（如 scan code）或扩展标志，
 *   虚拟键码本身并不总是唯一确定物理按键。
 *
